...
import { Ball, Brick, GameState, Paddle, Particle } from "./types";
...
export function updateGame(
  ...
  if (
    ...
    vy = -Math.sqrt(
      Math.max(INITIAL_BALL_SPEED * INITIAL_BALL_SPEED - vx * vx, 4),
    );
  ...
  for (let i = 0; i < bricks.length; i++) {
    ...
    if (distanceSquared <= r * r) {
      ...
      for (let p = 0; p < PARTICLE_COUNT; p++) {
        const angle =
          (Math.PI * 2 * p) / PARTICLE_COUNT + (Math.random() - 0.5) * 0.5; // Randomize angle slightly
        const spd =
          PARTICLE_BASE_SPEED +
          Math.random() * (PARTICLE_MAX_SPEED - PARTICLE_BASE_SPEED);
        newParticles.push({
          x: b.x + b.w / 2,
          y: b.y + b.h / 2,
      ...
      // Check which axis has the larger overlap distance
...
export function calculateNewBallVelocity(
  vx: number,
  vy: number,
  hitRatio: number,
): { vx: number; vy: number } {
